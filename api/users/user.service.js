...
module.exports = {
  ...
  notiGlobal: (callback) => {
    ...
    return callback(null, dataSys.textNotification);
  },
  ...
  addMoneyMember: (data, callback) => {
    db.query(
      ...
      [data.aUSDT, data.email],
      (error, results, fields) => {
        if (error) {
          return callback(error);
        }
        Tele.sendMessThongBao(`🧑ADMIN vừa thực hiện trừ tiền tới người dùng: <b>${data.email}</b>\n
                    USDT: <b>-${data.aUSDT}</b>`);
        return callback(null, results);
      }
    );
  ...
  getMoneyUser: (email, callback) => {
    db.query(
      ...
      (error, results, fields) => {
        ...
        return callback(null, results[0].balance);
      }
    );
  },

  updateMoneyUser: ({ email, newMoney }, callback) => {
    db.query(
      `update account set balance = ? WHERE email = ? and type = 1`,
      [newMoney, email],
      (error, results, fields) => {
        if (error) {
          return callback(error);
  ...
  WithDrawalNoiBo: (data, callback) => {
    ...
    db.query(
      ...
      (error, results, fields) => {
        ...
        console.log(phi)
        let tongPhi = Number(data.a) + Number(phi);
        console.log(tongPhi)
        if (results[0].money_usdt >= tongPhi) {
          //======= Từ tiền tài khoản mình
          db.query(
            `update users set money_usdt = money_usdt - ? where email = ?`,
            [tongPhi, data.email],             (error, results, fields) => {
              console.log(results)
            }
          );
          Tele.sendMessRut(
  ...
  WithDrawalVND: (data, callback) => {
    ...
    db.query(
      `select verified from users where email = ? AND nick_name = ?`,
      [data.email, data.nick_name],
      async (error, results, fields) => {
        if (error) {
          return callback(error);
        ...
        const money_usdt = await new Promise((resolve, reject) => {
          db.query(
            `select balance from account where email = ? AND type = 1`,
            [data.email],
            (error, results, fields) => {
  ...
  BalanceWallet: async (email, callback) => {
    const moneyUSDT = await new Promise((resolve, reject) => {
      db.query(
        `select balance from account where email = ? AND type = 1`,
        [email],
        (error, results, fields) => {
    ...
    db.query(
      ...
      (error, results, fields) => {
        ...
        return callback(null, Object.assign(results[0], { usdt: moneyUSDT }));
      }
    );
  },
