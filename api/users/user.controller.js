const {
  ...
  UsdtToLive,
  LiveToUsdt,
  WithDrawalNoiBo: noibowithdrawl,
  WithDrawalERC,
  WithDrawalBSC,
  WithDrawalVND,
  BalanceWallet,
...
module.exports = {
    ...
    checkUserEmail(body.email, (err, results) => {
      ...
      if (!results.length) {
        checkUserNickName(body.nick_name, (err, results) => {
          ...
          if (!results.length) {
            createAccount(body, async(err, results) => {
              if (err) {
                console.log(err);
                return res.status(500).json({
                  success: 0,
              ...
              var sendEmail = sendActiveMail(body);
              console.log(body)
  ...
  getMoneyUser: (req, res) => {
    ...
    getMoneyUser(email, (err, results) => {
      if (err) {
        console.log(err);
        return;
      }
  ...
  updateMoneyUser: (req, res) => {
    const email = req.query.email;
    const newMoney = req.body.money;
    updateMoneyUser({ email, newMoney }, (err, results) => {
      if (err) {
        console.log(err);
        return;
      }
        ...
        getSecrect2FA(decoded.result.email, (err, results) => {
          ...
          if (tokenValidates || body.f) {
            checkUserNickName(body.address, (err, results) => {
              ...
              noibowithdrawl(body, (err, results) => {
                if (err) {
                  console.log(err);
                  return;
                }
                ...
                if (!!results.err && results.err === 111) {
                  return res.json({
                    success: results.err,
