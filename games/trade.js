...
let RANDOM_SESSION = {}; // Random max 30 phiên
const RANDOM_SESSION_MAX = 6;
...
wss.on('connection', function(ws){
    ...
    ws.on('message', d => {
        ...
        if(data.type === 'editGL'){
            ...
            if (obj.type == 'REVERSE_USER_TRADE') {
                DATA_GL.REVERSE_USER_TRADE = obj.value;
            }
...
function countDownGame(){
    tCountDown = setInterval(() => {
        ...
        if(SO_GIAY_DEM_NGUOC === 0){
            ...
            // trở về giây cũ
            SO_GIAY_DEM_NGUOC = (ORDER_OR_WATTING === 'order' ? config.SO_GIAY_DEM_NGUOC_KET_QUA : config.SO_GIAY_DEM_NGUOC) + 1;
        ...
        jsonData['REVERSE_USER_TRADE'] = DATA_GL.REVERSE_USER_TRADE;
...
async function HandlingBuySell2(title){
    ...
    for(let obj in BTC_USER_SELL){
        ...
        if(action2 === title){ // đây là thắng của BUY
    ...
    // if (Number(keySessionRandom[keySessionRandom.length - 1]) === session - 1) {
        // RANDOM_SESSION = {};
        delete RANDOM_SESSION[session - 1];
        RANDOM_SESSION[session + RANDOM_SESSION_MAX] = ~~(Math.random() * 10) % 2 ? 'buy' : 'sell';
        // if (void 0 === RANDOM_SESSION[session + index]) {
        //     for (let index = 0; index <= RANDOM_SESSION_MAX; index++) {
        //         RANDOM_SESSION[session + index] = ~~(Math.random() * 10) % 2 ? 'buy' : 'sell';
        //     }
        // }
